...
Repurposing `UNK` can work with tokenizers that never produce `UNK`s in normal data (e.g. Mistral tokenizers should have
...
`UNK`s (padding) still get ignored. NOTE: This will leave the tokenizer's `pad_token_id` set to the `unk_token_id`; so
...
Hugging Face TRL `DPOConfig` as `Config` with additional SiloGen conventions
