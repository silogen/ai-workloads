# Fine-tuning config structure and parameters for SFT

This document describes the structure of the SFT fine-tuning configuration, and the parameters and values that can be defined there.

See the fine-tuning config section [this config file](overrides/llama-31-tiny-random-deepspeed-values.yaml) for an example of a valid configuration.
...
| Property | Type | Required | Possible values | Default | Description |
| -------- | ---- | -------- | --------------- | ------- | ----------- |
| `total_train_batch_size` | `integer` | ✅ | `intege`r |  | The total batch size for the training run |
...
The datasets themselves need to be in the fine-tuning supported JSONL formats.
...
| -------- | ---- | -------- | --------------- | ----------- |
| `path` | `string` | ✅ | `string` | Local path to a JSONL file in the fine-tuning data format |
...
input-side `UNK`s (padding) get set to `EOS`, so that the input-side padding looks like `EOS`. On the output-side, the
`UNK`s (padding) still gets ignored. NOTE: This will leave the tokenizer's `pad_token_id` set to the `unk_token_id`; so
...
- `attn_implementation`:
    ...
    the attention), `sdpa` (using `F.scaled_dot_product_attentioni`), or `flash_attention_2` (using
...
| `device_map` | `object` or `string` or `null` |  | `object` and/or `string` | `null` | Custom device map so that you can manually override the choices that Hugging Face would make. This can also be a string to specify `"auto"`, `"balanced_low_0"`, or `"sequential"`. |
...
| `tokenizer` | `string` or `null` |  | `string` | `null` | Model Hugging Face ID, or path, or `None` to use the one associated with the model |
...
| `save_name_if_new_basemodel` | `string` |  | `string` | `"checkpoint-new-basemodel"` | If a new base model is saved, it will be saved with this name |
| `train_on_completions_only` | `boolean` |  | `boolean` | `false` | Only compute loss on the assistant's turns. |

## SilogenTrainingArguments

Hugging Face `TrainingArguments` as `Config` with additional SiloGen conventions
...
These datasets are interleaved based on the sampling weights. The resulting dataset is fully precomputed, up to
